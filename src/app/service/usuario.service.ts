...
export class UsuarioService {
  ...
  public recuperaTodos(){
    return this.http.get("http://31.220.49.115:8080/cadastro/todos/");
  }


  public recuperaPorToken(token:String){
    return this.http.get("http://31.220.49.115:8080/cadastro/detalhes?token="+token);
  }
  //puxando informações da api de usuários do Isidro (puxando informações por ID)
  public recuperaDetalhe(id: number){
    return this.http.get("http://31.220.49.115:8080/cadastro/" + id);
  }

  //inserir novo usuario
  //metodo http, via post
  public insere(usuario: CadastroUsuario){
    return this.http.post("http://31.220.49.115:8080/cadastro/novo/", usuario);
  }

  //alterar informações dos usuários
  //método http, via put 
  public atualiza(usuario: CadastroUsuario){
    return this.http.put("http://31.220.49.115:8080/cadastro/novo", usuario);
  }

  public login(usuario: CadastroUsuario){
    return this.http.post("http://31.220.49.115:8080/login/", usuario);
  }
