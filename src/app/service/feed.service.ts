...
export class FeedService {
  ...
  novaPubli(feed: Postagem){
    ...
    return this.http.post("http://31.220.49.115:8080/postagem/novo/",feed);
  }

  // Serviço para alimentar o Feed de noticia
  getAll(){
    let token = localStorage.getItem("eurekaToken");
    return this.http.get("http://31.220.49.115:8080/postagem/todos/?token=" + token);
  }

  getOne(id: number){
    return this.http.get("http://31.220.49.115:8080/postagem/"+id);
  }
  editar(id : number){
    return this.http.put("http://31.220.49.115:8080/postagem/" , id);
  }
